# -*- coding: utf-8 -*-
"""
Created on Thu Feb 10 19:10:34 2022
...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 10 18:31:11 2022
...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #image = Image.fromarray(np.double(np.load(img_path)))
        #image = Image.fromarray(np.load(img_path))
        image = npy_loader(img_path))
...
def fit_model(model, loss_func, optimizer, input_shape, num_epochs, train_loader, test_loader):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch+1)%5 == 0:
            torch.save(model,'{}Epoch'.format(epoch+1)+'.pt')
